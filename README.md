...
```
sns.scatterplot(x=df['Age'],y=df["Fare"])a
```
...
Countplots and catplots to study categorical variables like Sex and Pclass.
Boxplots to detect outliers and visualize Age distribution by Survival and Class.
Scatterplots to examine the relationship between Age and Fare.
Pairplots and heatmaps to explore correlations among numerical variables.
Data Transformation: Renamed columns for clarity (e.g., sex → Gender) and set PassengerId as the index.
